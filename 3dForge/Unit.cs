...
namespace _3dForge
{
    ...
    {  
        ...
        {          
            ...
            {
                //frozenTimer = Clamp(frozenTimer - (1.0f*Time.DeltaTime), 0, frozenMaxTime);
            }
        ...
        {
            curHealth = Clamp(curHealth + healthRegeneration, 0, maxHealth);
            curShield = Clamp(curShield + shieldRegeneration, 0, maxShield);
            curArmor = Clamp(curArmor + armorRegeneration, 0, maxArmor);
        }
        ...
        private void CalcDamage(DamageType damageType, float damage)
        {
            ...
            {
                float damageToShield = Clamp(damage, 0, curShield);
                damage -= damageToShield;
                curShield -= damageToShield;
            }

            //Schadensberechnung Rüstung
            float damageToArmor = Clamp(damage, 0, curArmor);
            damage -= damageToArmor;
            curArmor -= damageToArmor;

            //Wiederstand
            damage -= Clamp(damage - maxResistance, 0, damage);

            //Schadensberechnung Normal
            curHealth -= Clamp(damage, 0, curHealth);


            /*
            if (healthPoints <= 0) gameObject.Destroy()
